...
`./scripts/fetch_models.sh`, then all **497 offline tests** collected and 489
...
```bash
pytest -m "not network"   # 497 offline tests, 97% line coverage
pytest -m network         # 3 tests against the live API and a live chain
```

Both installs collect the same **497 offline tests**. Every insightface gate in
...
|---|---|---|---|
| Quickstart `.[insight,dev]` | 496 | 1 | **97%** |
| CI `.[dev]` | 489 | 8 | **96%** |
...
That 497 is a 3.11-and-up figure. On 3.10 it is 489, because `tomllib` is not
