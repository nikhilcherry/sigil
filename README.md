...
`./scripts/fetch_models.sh`, then all **496 offline tests** collected and 488
...
```bash
pytest -m "not network"   # 496 offline tests, 97% line coverage
pytest -m network         # 3 tests against the live API and a live chain
```

Both installs collect the same **496 offline tests**. Every insightface gate in
...
|---|---|---|---|
| Quickstart `.[insight,dev]` | 495 | 1 | **97%** |
| CI `.[dev]` | 488 | 8 | **96%** |
...
That 496 is a 3.11-and-up figure. On 3.10 it is 488, because `tomllib` is not
